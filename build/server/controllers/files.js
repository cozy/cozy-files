...
var File, Folder, async, fs, log, moment, processAttachement, sharing;
  ...
  if (!req.body.name || req.body.name === "") {
    ...
    return File.all((function(_this) {
      return function(err, files) {
        var available, avalailable, data, file, fileDoc, fileFullPath, fullPath, now, _i, _len;
        avalailable = true;
        fullPath = "" + req.body.path + "/" + req.body.name;
        for (_i = 0, _len = files.length; _i < _len; _i++) {
          fileDoc = files[_i];
          fileFullPath = "" + fileDoc.path + "/" + fileDoc.name;
          if (fullPath === fileFullPath) {
            available = false;
          }
        }
        if (available) {
