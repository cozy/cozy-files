...
var File, Folder, async, folderParent, fs, getFileClass, log, mime, moment, multiparty, normalizePath, pathHelpers, processAttachement, resetTimeout, sharing, timeout, updateParents,
      ...
      if (!name || name === "") {
        ...
        return File.byFullPath({
          ...
          return function(err, sameFiles) {
            var createFile, data, fileClass, now;
            if (err) {
              return next(err);
            }
            if (sameFiles.length > 0) {
              ...
              createFile = function() {
                return File.createNewFile(data, part, function(err, newfile) {
                  var stringErr, who;
                  resetTimeout();
                  if (err) {
